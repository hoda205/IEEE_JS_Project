...
async function createAppointment(patientUserId, doctorId = "1") {
  try {
    ...
    if (activeAppointment) {
      ...
      window.location.href = `queue-tracking.html?appointmentId=${activeAppointment.id}`;
      return;
    }
    const now = new Date();
    ...
    // صفحة متابعة الحجز بقي

    window.location.href = `queue-tracking.html?appointmentId=${savedApp.id}`;
  } catch (error) {
